...
_end_date_cache: dict  = {}  # slug → end_date_str
# Bounded 2026-08-25: was process-lifetime and unbounded, growing one entry per
# distinct slug forever. Values are immutable per slug, so a simple size cap is
# sufficient — no TTL needed.
_END_DATE_CACHE_MAX = 2000
...
def _fetch_event_end(slug: str) -> str | None:
    ...
    while len(_end_date_cache) > _END_DATE_CACHE_MAX:
        _end_date_cache.pop(next(iter(_end_date_cache)), None)
    return end
