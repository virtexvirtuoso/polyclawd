...
def _arb_signals(report: dict) -> list[dict]:
    ...
    # Staleness gate: Kalshi prices >48h old generate phantom arb signals.
    # Suppress until Kalshi data is refreshed by auth-enabled fetch.
    try:
        ...
        if row and row[0]:
            ...
            last_kalshi = datetime.strptime(row[0], "%Y-%m-%d").replace(tzinfo=timezone.utc)
            kalshi_age_hours = (datetime.now(timezone.utc) - last_kalshi).total_seconds() / 3600
            if kalshi_age_hours > 48:
                logger.warning(
                    "Election arb suppressed: Kalshi DB is %.0fh stale (last: %s)",
                    kalshi_age_hours, row[0]
                )
                return []
    except Exception as e:
        logger.debug("Staleness gate check failed: %s", e)
