...
def check(label, cond):
    # PYTEST_CHECK_RAISES_2026_08_26: this used to only count and print. Under pytest that made
    # every test in this file pass unconditionally (verified with a deliberate
    # failing check that still reported "1 passed"). It must raise.
...
# PYTEST_CHECK_RAISES_2026_08_26: the module-level runner + `sys.exit()` below used to run
# here at IMPORT time. pytest hit the SystemExit and aborted the whole suite with
# INTERNALERROR -- every other test file included. pytest collects the `test_*`
# functions above directly, so the runner is not needed.
