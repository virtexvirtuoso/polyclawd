...
def resilient_call(source_name: str, fn, retries: int = 2, backoff_base: float = 2.0, default=None):
    ...
    for attempt in range(retries + 1):
        ...
        try:
            ...
            # Fail fast on HTTP 4xx (except 429): the resource doesn't exist or
            # the request is invalid — retrying can't succeed and just burns time
            # (e.g. CLOB /book 404s were retried 3× at connect-timeout cost each).
            status = getattr(e, "code", None)  # urllib HTTPError
            if status is None:
                status = getattr(getattr(e, "response", None), "status_code", None)  # httpx/requests
            if isinstance(status, int) and 400 <= status < 500 and status != 429:
                logger.warning("resilient: %s HTTP %d — not retrying", source_name, status)
                break
    ...
    # Report attempts ACTUALLY made, not the configured ceiling. The old message
    # hardcoded `retries + 1`, so a single non-retryable 404 logged "ALL 3 attempts
    # failed" — which read as a retry storm and sent a prior investigation chasing
    # a retry ladder that was never running.
    logger.error(
        "resilient: %s failed after %d/%d attempt(s): %s",
        source_name,
        attempts_made,
        retries + 1,
        last_error,
