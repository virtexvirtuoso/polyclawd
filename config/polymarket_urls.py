...
# Origins that stay direct even when PROXY_ENABLED.
#
# clob.polymarket.com sits behind a Cloudflare WAF that answers Worker
# subrequests with HTTP 403 / CF error 1010 ("banned browser signature") when
# the subrequest egresses from certain colos — including SIN, the colo that
# serves our Hetzner VPS. Measured from the VPS on 2026-08-25:
#     worker /clob/*                      -> 403 (CF error 1010)
#     worker /gamma|/data|/xtracker/*     -> 200
#     direct  clob.polymarket.com/*       -> 200
#     worker /clob/* from an NRT colo     -> 200
# The Worker is deployed, correctly routed and needs no auth — only this one
# upstream rejects it, and only from our colo. So clob reads stay on the
# WireGuard tunnel while the other three keep the CF edge cache.
_NEVER_PROXY: Final[frozenset] = frozenset({"clob"})

# When proxy is disabled, reads use the origin directly.
# When enabled, reads use POLYPROXY_BASE/<prefix>/<path>.


def _proxy_prefix(origin_key: str) -> str:
    """Worker path prefix for an upstream origin key ('gamma-api' -> 'gamma')."""
    return origin_key.split("-")[0]
...
def _make_read_urls(origin_key: str) -> tuple[str, str]:
    ...
    if PROXY_ENABLED and origin_key not in _NEVER_PROXY:
        proxied = f"{_POLYPROXY_BASE}/{_proxy_prefix(origin_key)}"
    else:
        proxied = direct
...
# Proxied-base → direct-origin map, so a caller whose Worker read failed can
# retry the same request against the upstream. Empty when the proxy is off or
# when every origin is in _NEVER_PROXY (nothing to fall back from).
PROXY_FALLBACKS: Final[dict] = {
    f"{_POLYPROXY_BASE}/{_proxy_prefix(_k)}": _v
    for _k, _v in _READ_ORIGINS.items()
    if PROXY_ENABLED and _k not in _NEVER_PROXY
}


def direct_equivalent(url: str) -> Optional[str]:
    """Map a Worker-proxied read URL back to its direct upstream URL.

    Returns None when the URL is not proxied — i.e. there is no second route
    to try, so the caller should treat the original failure as terminal.
    """
    for proxied, direct in PROXY_FALLBACKS.items():
        if url.startswith(proxied):
            return direct + url[len(proxied) :]
    return None
...
def assert_safe_url(url: str, method: str = "GET") -> None:
    ...
    if PROXY_ENABLED and parsed.startswith(_POLYPROXY_BASE):
        if "/relayer/" in parsed or parsed.endswith("/relayer"):
            raise RuntimeError(f"relayer path routed through Worker — this must not happen: {url}")
    ...
    if method in _WRITE_METHODS and PROXY_ENABLED and parsed.startswith(_POLYPROXY_BASE):
        ...
        raise RuntimeError(f"{method} write routed through Worker — writes must go via tunnel: {url}")
    ...
    if PROXY_ENABLED and parsed.startswith(_POLYPROXY_BASE):
        for wp in _WRITE_CLOB_PATHS:
            if wp in parsed:
                raise RuntimeError(f"write-path {wp} routed through Worker — must stay on tunnel: {url}")
...
def status() -> dict:
    ...
    return {
        ...
        "never_proxy": sorted(_NEVER_PROXY),
        "gamma_api": GAMMA_API,
        "clob_api": CLOB_API,
...
def _patch_polymarket_sdk() -> None:
    ...
    try:
        ...
        logging.getLogger("polyproxy").warning("Failed to monkey-patch polymarket-client SDK: %s", e)
