...
# === EVERY 5 MIN: Paper portfolio resolution ===
$VENV -c "
...
# === EVERY 5 MIN: Weather position re-evaluation (same-day positions) ===
$VENV -c "
from signals.weather_scanner import reeval_weather_positions
reeval_weather_positions()
" > /dev/null 2>&1 || true

# === EVERY 30 MIN: Signal scan + paper portfolio (category + weather) ===
MINUTE=$(date -u +%M)
MOD=$((10#$MINUTE % 30))
if [ "$MOD" -lt 10 ]; then
    # Category signals (existing)
    $VENV -c "
...
" > /dev/null 2>&1 || true
    ...
    logger -t polyclawd-watchdog "Signal scan + portfolio processing complete (category + weather)"
fi
