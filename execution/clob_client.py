...
# ---------------------------------------------------------------------------
# Vendor minimum order size — PER MARKET, denominated in SHARES
# ---------------------------------------------------------------------------
#
# Polymarket rejects any order whose size (in conditional-token shares) is below
# the market's `min_order_size`. It is NOT a global constant: it is a per-market
# field exposed as
#     GET /book?token_id=...        -> "min_order_size"   (CLOB, used here)
#     GET /markets                  -> "minimum_order_size"
#     GET gamma /markets            -> "orderMinSize"
# Verified live 2026-08-25: 400/400 sampled OPEN markets return 5, but the CLOB
# market dump also contains 0 / 14 / 14.96 / 14.97 / 15 on legacy markets — so
# the value must be READ per market, never hardcoded.
#
# Units are SHARES, not USD: a book level's `size` is a share count, and the
# same market quotes last_trade_price 0.013 with min_order_size 5 (a $0.065
# notional), which only makes sense as a share count.
#
# DEFAULT_MIN_ORDER_SHARES is the fallback when the vendor cannot be reached.
# It matches the value every currently-open market reports.
DEFAULT_MIN_ORDER_SHARES = 5.0

_MIN_ORDER_SIZE_TTL = 300.0  # seconds; the field effectively never changes
_min_order_size_cache: dict = {}


def get_min_order_size(token_id: str) -> float:
    """Return the vendor minimum order size for *token_id*, in SHARES.

    Reads the live CLOB order book (no auth required) and caches per token for
    _MIN_ORDER_SIZE_TTL seconds. Never raises: on any failure it logs a WARNING
    and returns DEFAULT_MIN_ORDER_SHARES, so a transient vendor outage cannot
    turn the safety guard into a crash.

    POLYCLAWD_MIN_ORDER_SHARES env var overrides the lookup entirely (ops
    escape hatch if Polymarket changes the field name).
    """
    override = os.environ.get("POLYCLAWD_MIN_ORDER_SHARES", "").strip()
    if override:
        try:
            return float(override)
        except ValueError:
            logger.warning("POLYCLAWD_MIN_ORDER_SHARES=%r is not a number; ignoring", override)

    # Never hit the vendor during test runs (repo convention, cf.
    # alert_formatter._send_telegram_inner) — unit tests must be offline and
    # deterministic. Tests that need a non-default minimum monkeypatch
    # live_executor._min_order_shares.
    if os.environ.get("PYTEST_CURRENT_TEST"):
        return DEFAULT_MIN_ORDER_SHARES

    import time as _time

    now = _time.monotonic()
    hit = _min_order_size_cache.get(token_id)
    if hit is not None and (now - hit[1]) < _MIN_ORDER_SIZE_TTL:
        return hit[0]
    ...
    try:
        ...
        resp.raise_for_status()
        raw = resp.json().get("min_order_size")
        if raw is None:
            raise ValueError("no min_order_size field in /book response")
        val = float(raw)
        if val <= 0:
            raise ValueError(f"non-positive min_order_size {val!r}")
    except Exception as exc:
        logger.warning(
            "get_min_order_size(%s): vendor lookup failed (%s) — falling back to %.2f shares",
            token_id[:16],
            exc,
    ...
    _min_order_size_cache[token_id] = (val, now)
    return val
